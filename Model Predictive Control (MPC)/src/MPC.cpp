...
double ref_epsi = 0;
// Cost factors 
int cost_cte_factor = 100;
int cost_epsi_factor = 500;
int cost_v_factor = 1;
int cost_delta_factor = 1;
int cost_a_factor = 10;
int cost_delta_factor_d = 300;
int cost_a_factor_d = 300;
//Init state - actuator variables 
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Distance effect in cost function
    ...
    // Actuator effect in cost function
    for (unsigned int i = 0; i < N - 1; i++) {
      fg[0] += cost_delta_factor*CppAD::pow(vars[delta_start + i], 2);
      fg[0] += cost_a_factor*CppAD::pow(vars[a_start + i], 2);
    }
    // Sequential actuators effect in cost function
    for (unsigned int i=0; i < N-2; i++) {
      fg[0] += cost_delta_factor_d*pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += cost_a_factor_d*pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
   ///////////////////////////////////////////////CONSTRAINTS/////////////////////////////////////////
   ...
   /////////////////////////////////////////////////STATES///////////////////////////////////////////
    for (unsigned int i = 1; i < N; i++) {
      ...
      // Previous timesteps
      if (i > 1) // uses previous states for latency compensation
      {
        a = vars[a_start + i - 2];
      ...
      // Actual- predicted states differences
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Steering angle limits -25 and 25 (in radians)
